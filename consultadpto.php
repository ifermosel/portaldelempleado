<?php
	...
	if ($cargo != 'Manager') {
		echo "<center><h1>Historial de tus departamentos</h1></center>";
		...
		$sql2 = "SELECT dept_name, from_date, to_date FROM departments, dept_emp WHERE departments.dept_no = dept_emp.dept_no AND emp_no = '$codigoEmp'";
		...
		if(isset($_REQUEST['analizar'])) {
			if (!empty($_REQUEST['codigo'])) {
				...
				if(empty($hola)) {
					...
					$sql = "SELECT dept_name, from_date, to_date FROM departments, dept_emp WHERE departments.dept_no = dept_emp.dept_no AND dept_emp.emp_no = '$codigo'";
					$query = mysqli_query($db, $sql);
					echo "<table border='1' padding='10' style='margin: 0 auto;'>";
					echo "<tr><th>Nombre del departamento</th><th>Fecha inicial</th><th>Fecha final</th></tr>";
