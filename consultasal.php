<?php
	...
	if ($cargo != 'Manager') {
		echo "<center><h1>Historial de tus salarios</h1></center>";
		...
		$sql2 = "SELECT salary, from_date, to_date FROM salaries WHERE emp_no = '$codigoEmp'";
		...
		if(isset($_REQUEST['analizar'])) {
			if (!empty($_REQUEST['codigo'])) {
				...
				if(empty($hola)) {
					...
					$sql = "SELECT salary, from_date, to_date FROM salaries WHERE emp_no = '$codigo'";
					$query = mysqli_query($db, $sql);
